...
const getLegacyUci = rpc.declare({
	object: 'luci.sftpgo',
	method: 'get_legacy_uci',
	expect: {}
});

const migrateLegacyUci = rpc.declare({
	object: 'luci.sftpgo',
	method: 'migrate_legacy_uci',
	expect: {}
});
...
return view.extend({
	load: function () {
		return Promise.all([
			getNativeConfig(),
			L.resolveDefault(getLegacyUci(), { available: false })
		]);
	},
	...
	render: function (data) {
		...
		function validate() {
			return validateNativeConfig(currentContent()).then(function (res) {
				...
				notify(_('JSON validation failed: ') + ((res && res.error) || _('Unknown error')), 'error');
				return false;
			}).catch(function (err) {
		...
		function save(restart) {
			return validate().then(function (valid) {
				...
				return saveNativeConfig(currentContent()).then(function (res) {
					if (!res || !res.ok) {
						notify(_('Save failed: ') + ((res && res.error) || _('Unknown error')), 'error');
						return false;
					}
		...
		function resetDefault() {
			...
			return resetNativeConfig().then(function (res) {
				if (!res || !res.ok) {
					notify(_('Reset failed: ') + ((res && res.error) || _('Unknown error')), 'error');
					return;
				}
				return reload();
			}).catch(function (err) {
		...
		function migrate() {
			if (!confirm(_('Copy the old LuCI UCI listener/database settings into the current sftpgo.json? The existing JSON will be backed up first.')))
				return;

			return migrateLegacyUci().then(function (res) {
				if (!res || !res.ok) {
					notify(_('Migration failed: ') + ((res && res.error) || _('Unknown error')), 'error');
					return;
				}
				return reload();
			}).catch(function (err) {
				notify(_('Migration failed: ') + (err.message || err), 'error');
			});
		}
		...
		var migrationBox = null;
		if (legacy.available) {
			migrationBox = E('div', { class: 'alert-message warning' }, [
				E('strong', {}, _('Legacy UCI settings detected.')),
				E('p', {}, _('The previous LuCI version stored listener ports and database settings in /etc/config/sftpgo. The new version uses sftpgo.json for these values. Migrate once, then use only the native JSON configuration.')),
				E('button', { class: 'cbi-button cbi-button-apply', click: function () { migrate(); } }, _('Migrate old UCI settings to JSON'))
			]);
		}
		...
		return E('div', { class: 'cbi-map' }, [
			E('div', { class: 'cbi-section' }, [
				...
				E('div', { id: 'sftpgo_native_status', class: 'alert-message notice', style: 'display:block;' },
					info.valid ? _('Configuration loaded successfully.') : (_('Current JSON is invalid: ') + (info.error || _('unknown error')))),
