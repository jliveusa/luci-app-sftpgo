...
return view.extend({
	load: function () {
		return Promise.all([
			getNativeConfig(),
			L.resolveDefault(getConfigSummary(), { valid: false, runtime_uci_overrides: false })
		]);
	},
	...
	render: function (data) {
		...
		var legacyNotice = null;
		if (summary.legacy_uci_ignored) {
			legacyNotice = E('div', { class: 'alert-message warning' }, [
				E('strong', {}, _('Old UCI protocol/database keys were detected but are ignored.')),
				E('p', {}, _('They do not override this file and are not passed to SFTPGo. The native sftpgo.json shown below is the authoritative runtime configuration.'))
			]);
		}
