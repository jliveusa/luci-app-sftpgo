...
return view.extend({
	...
	render: function (data) {
		...
		s.render = function () {
			...
			nodes.push(E('small', { style: 'display:block; margin-top:0.25em; opacity:0.8;' },
				_('Runtime configuration source: native sftpgo.json (UCI protocol/database overrides are disabled).')));

			if (summary.legacy_uci_ignored) {
				nodes.push(E('div', { class: 'alert-message warning', style: 'margin-top:0.75em;' },
					_('Old UCI protocol/database keys still exist from an earlier package version, but this version ignores them completely. Running SFTPGo uses only the native JSON configuration.')));
			}
